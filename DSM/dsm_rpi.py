...
FSANET_MODEL_PATH = "fsanet.tflite" 
EYE_MODEL_PATH    = "eye_cnn_int8.tflite"
MOUTH_MODEL_PATH  = "mouth_cnn_int8.tflite"
MASK_MODEL_PATH   = "mask_cnn_int8.tflite"
...
class RKNNHeadPose:
    ...
    def predict(self, face_rgb_64x64):
        inp = np.ascontiguousarray(face_rgb_64x64, dtype=np.float32)[np.newaxis, ...] / 255.0
        self.interp.set_tensor(self.inp_idx, inp)
        self.interp.invoke()
        res = self.interp.get_tensor(self.out_idx)[0]
...
class RKNNModelEye:
    ...
    def predict(self, img_float32):
        inp = img_float32.astype(np.float32)[np.newaxis]
        self.interp.set_tensor(self.inp_idx, inp)
        self.interp.invoke()
...
def npu_thread():
    ...
    print("🧠 [Thread C] TFLite Worker Thread Started")
